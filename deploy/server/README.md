...
비밀번호의 **길이·문자 조합 제한은 없지만 빈 값은 허용하지 않습니다.** 입력 중 화면에 글자나 별표가 표시되지 않는 것이 정상입니다. 빈 값이면 확인 입력 전에 이유를 표시하고, 확인 불일치면 두 값을 다시 받습니다. 최대 3회 시도 후에는 상태 파일을 만들지 않고 종료합니다. 비밀번호를 명령 인자·채팅·로그에 넣지 마세요. 길이 제한을 없앤 것은 보안 강도를 보장한다는 뜻이 아닙니다. 포털 접근은 반드시 승인된 IP/VPN으로 제한합니다. 이미 생성한 비밀번호·해시·인증서는 변경하지 않으며 해시 저장과 인증은 유지합니다.
...
명령은 포털 `admin` 계정의 비밀번호를 대화형으로 두 번 요청합니다. 길이·문자 조합 제한 없이 빈 값이 아닌 동일한 값을 입력합니다. 비밀번호를 명령 인자, 셸 히스토리, Git에 넣지 않습니다. 이미 저장소가 있다면 새로 복제할 필요가 없습니다.
...
포털 DB는 [백업·격리 복원 도구](../../docs/portal_backup.md)로 명시적으로 보관할 수 있습니다. 실행 중 SQLite의 일관된 사본을 만들고 새 경로 복원만 허용합니다. 전체 서버 백업/운영 자동 복구 기능은 아닙니다.
