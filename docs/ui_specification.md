...
- Header: Cloud SOC, 현재 Workspace, Production **(DEMO)**, UTC+9 / Asia/Seoul, 현재 UI 시각, 사용자 아이콘.
- Sidebar: 기본 204px, 고정 배치. Mission Control, Investigation, Entity, Hunting, Detection Coverage, System Health, Kibana, Settings 순서.
- 고정 배너: `PHASE A UI PROTOTYPE`, `DEMO DATA`, `NO BACKEND CONNECTION` 및 Demo State 선택기.
- 실제 화면은 Mission Control과 Workbench뿐이다. 나머지 메뉴와 사용자 아이콘은 설명 모달이다.
...
두 화면 모두 고정 Demo State 선택기로 아래 상태를 확인한다. 외부 장애/권한을 실제 측정하거나 재현하는 기능은 아니다.
