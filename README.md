...
**개발 진행 상황은 [우선순위별 작업 목록](docs/work_tracker.md)에서 관리합니다.** 작업별 완료 기준을 확인하고, 구현·테스트·배포·실제 수신 검증을 구분하여 완료 표시합니다. 다른 PC에서 이어서 작업할 때도 이 목록부터 확인하세요.
...
새 Windows 패키지는 [SYSTEM 사전 검사와 staging 설치](docs/windows_install_recovery.md)를 적용합니다. 준비 단계 실패는 소유 임시 자원만 정리하며, NIC는 확인 후 선택합니다. 기존 잔존 설치의 자동 Repair·Enrollment·수신 확인까지 구현된 것은 아닙니다.
...
**중앙 포털의 통합 로그 화면은 실제 조회 API와 연결되어 있습니다.** 기간·호스트·OS·수집기·IP로 메타데이터를 조회하고 문서 상세를 확인합니다. 본문·전체 원문은 보호 정책 검증 전까지 제한됩니다. 배포와 사용법은 [통합 로그 안내](docs/log_explorer.md)를 확인하세요. 로컬 코드 구현과 AWS 반영은 별도입니다.

수집 확대 전에는 [P2 수집 보호·품질 가이드](docs/collection_protection.md)를 확인하세요. 새 에이전트의 민감정보 마스킹, 중앙 **수집 품질** 화면, 읽기 전용 저장량 보고, 로컬 경로 정책 갱신/복구를 제공합니다. 기존 패키지·키·에이전트는 자동 갱신되지 않으며, 운영 보존 삭제와 백업은 아직 자동 적용하지 않습니다.

[P3 호스트 보안 감사](docs/host_security_audit.md)의 1차 구현으로 **통합 로그 → 문서 ID → 보안 이벤트 해석**을 추가했습니다. 지원되는 Windows 보안/Sysmon 및 네트워크 메타데이터를 표시합니다. Linux SSH는 선택적 P5 처리기로 연결하며, Linux Audit 다중 레코드 결합·OS 감사 자동 활성화·실제 배포는 아직 미완료입니다.

[P4 AWS CloudTrail 가이드](docs/aws_cloudtrail.md)는 선택적 **관리 이벤트 수집기**와 통합 로그 연결을 설명합니다. 계정·리전 명시, 체크포인트·재전송 중복 방지, 별도 최소 권한 ES 키를 사용합니다. 기본 CLI 실행은 설정 검증만 하며 `--run`을 지정해야 실제 수집합니다. S3 객체 접근 등 데이터 이벤트와 AWS 설정 변경은 자동 활성화하지 않습니다.

[OCI Audit 가이드](docs/oci_audit.md)도 추가했습니다. 테넌시·리전·구획을 지정한 별도 수집기와 통합 로그의 `OCI Audit` 필터를 사용합니다. 기본은 오프라인 검증이며 실제 인증·수집은 명시적 실행이 필요합니다. OCI Object Storage 객체 접근/Identity Domains 감사는 이번 범위에 포함하지 않습니다.
